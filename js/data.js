...
(() => {
  ...
  var Store = {
    load() {
      ...
      let raw = null;
      try {
        raw = JSON.parse((_a = localStorage.getItem(DATA_KEY)) != null ? _a : "null");
      } catch {
        raw = null;
      }
      if (!raw) {
      ...
      for (const key of Object.keys(DEFAULT_DATA)) {
        if (!Array.isArray(merged[key])) {
          merged[key] = clone(DEFAULT_DATA[key]);
        }
      }
      cache = merged;
      return cache;
    ...
    getById(collection, id) {
      ...
      return (_a = items.find((item) => item.id === id)) != null ? _a : null;
    },
    add(collection, item) {
      const created = { ...item, id: generateId(collection) };
      this.load()[collection].push(created);
      this.save();
      return created;
    },
  ...
  var Applications = {
    getAll() {
      var _a, _b;
      try {
        return (_b = JSON.parse((_a = localStorage.getItem(APPLICATIONS_KEY)) != null ? _a : "null")) != null ? _b : [];
      } catch {
        return [];
      }
    },
